...
PTR(Val) _let::interp() {
    ...
    PTR(Val) rhsVal = rhs->interp();

    //create a number with the value of RHS
    PTR(Expr) rhsNum = rhsVal->to_expr();

    //Substitute body with rhs
    PTR(Expr) substitutedBody = this->body->subst(this->lhs, rhsNum);
...
PTR(Val) IfExpr::interp() {
    ...
    PTR(BoolVal) conditionVal = CAST(BoolVal)(condition->interp());
